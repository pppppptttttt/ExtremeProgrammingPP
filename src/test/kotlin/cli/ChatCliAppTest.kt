...
class ChatCliAppTest {
    @Test
    fun `server mode starts server and disconnects on exit`() =
        runBlocking {
    ...
    @Test
    fun `client mode connects sends message and prints sent message`() =
        runBlocking {
            val transport = FakeChatTransport()
            val out = ByteArrayOutputStream()
            val err = ByteArrayOutputStream()
    ...
    @Test
    fun `help command does not send anything`() =
        runBlocking {
            val transport = FakeChatTransport()
            val out = ByteArrayOutputStream()
            val err = ByteArrayOutputStream()
